...
Release archives, checksum files, the VS Code extension package, and
per-artifact CycloneDX SBOMs are all attested with GitHub artifact attestations
...
- `tnix-<version>-linux-x64.cdx.json`
- `tnix-<version>-macos-arm64.cdx.json`
- `tnix-vscode-<version>.cdx.json`

The SBOMs are generated by [`anchore/sbom-action`](https://github.com/anchore/sbom-action)
(Syft under the hood) and pinned to a full commit SHA. Each SBOM is attested
via [`actions/attest`](https://github.com/actions/attest) at the same step as
